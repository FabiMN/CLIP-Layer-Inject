...
class ImageNet():
    ...
    def __init__(self, root, num_shots, preprocess):
        ...
        val_imgs, val_targets, test_imgs, test_targets = self.split_val_into_val_test(self.val.imgs)
        self.val.imgs = val_imgs
        self.val.targets = val_targets
        self.val.samples = val_imgs
        self.test.imgs = test_imgs
        self.test.targets = test_targets
        self.test.samples = test_imgs

    def split_val_into_val_test(self, original_val):
        append_to_val_data = True
        val_imgs = []
        val_targets = []
        test_imgs = []
        test_targets = []

        for entry in original_val:
            if append_to_val_data:
                val_imgs.append(entry)
                val_targets.append(entry[1])
            else:
                test_imgs.append(entry)
                test_targets.append(entry[1])
            append_to_val_data = not append_to_val_data

        return val_imgs, val_targets, test_imgs, test_targets
