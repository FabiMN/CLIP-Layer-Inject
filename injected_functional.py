"""This is a modified version of the PyTorch implementation
"""
...
from torch._C._nn import pad, linear
...
def multi_head_attention_forward(
    ...
    if not use_separate_proj_weight:
        ...
        if injected_q:
            q_injected = injected_q(query)
            q = q + q_injected
        if injected_k:
            k_injected = injected_k(key)
            k = k + k_injected
        if injected_v:
            v_injected = injected_v(value)
            v = v + v_injected
    ...
    attn_output_input = attn_output.transpose(0, 1).contiguous().view(tgt_len * bsz, embed_dim)
    attn_output = linear(attn_output_input, out_proj_weight, out_proj_bias)
    if injected_out_proj:
        out_proj_injected = injected_out_proj(attn_output_input)
        attn_output = attn_output + out_proj_injected
